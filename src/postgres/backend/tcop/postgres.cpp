...
SocketBackend(StringInfo inBuf)
{
  ...
  switch (qtype)
  {
    ...
    case 'S':				/* sync */
      ...
      ignore_till_sync = false;
      /* mark not-extended, so that a new___ error doesn't begin skip */
      doing_extended_query_message = false;
...
exec_simple_query(const char *query_string)
{
   ...
   * BEGIN/COMMIT/ABORT statement; we have to force a new___ xact command after
  ...
  foreach(parsetree_item, parsetree_list)
  {
    ...
    *plantree_list;
    ...
    if (IsA(parsetree, TransactionStmt))
    {
       ...
       * start a new___ xact command for the next command (if any).
       */
      finish_xact_command();
    }
...
exec_execute_message(const char *portal_name, long max_rows)
{
  ...
  if (completed)
  {
    if (is_xact_command)
    {
       ...
       * will start a new___ xact command for the next command (if any).
       */
      finish_xact_command();
    }
    else
...
quickdie(SIGNAL_ARGS)
{
   ...
   * would queue new___ interrupts, one may have been pending.  We don't want a
